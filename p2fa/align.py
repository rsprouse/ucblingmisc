...
def prep_wav(orig_wav, out_wav, sr_override, wave_start, wave_end):
	...
	# Changed by RLS.
        ...
        shutil.copyfile(orig_wav, out_wav)
...
if __name__ == '__main__':
	...
	# Changed by RLS.
	# create ./tmp/dict with default values and append local dictionary, if provided
	shutil.copyfile(base_dict, word_dictionary)
	if os.path.exists(local_dict):
		with open(word_dictionary, 'a') as worddict:
			with open(local_dict, 'rb') as localdict:
				for line in localdict.readlines():
					worddict.write(line)
	...
	tmpwav = os.path.join(".", "tmp", "sound.wav")
